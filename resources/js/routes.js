...
export default [
...

    {
        path: '/login',
        name: 'login',
        component: require('./screens/authentication/login').default,
        meta: {
            fullPage: true,
            centeredPage: true
        }
...

    {
        ...
        component: {
            beforeCreate: async () => {
                const response = await axios.delete(Telescope.basePath + '/telescope-api/session')
                
                if (response.status == 204) {
                    sessionStorage.removeItem('telescope.currentApplication')
                    window.location.replace("/telescope/login")
                }
            },
