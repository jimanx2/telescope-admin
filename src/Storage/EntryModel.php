...
class EntryModel extends Model
{
    ...
    public function scopeWithTelescopeOptions($query, $type, EntryQueryOptions $options)
    {
        $this->whereType($query, $type)
                ->whereApplicationUuid($query, $options)
    ...
    /**
     * Scope the query for the given application Uuid.
     *
     * @param  \Illuminate\Database\Eloquent\Builder  $query
     * @param  \Laravel\Telescope\Storage\EntryQueryOptions  $options
     * @return $this
     */
    protected function whereApplicationUuid($query, EntryQueryOptions $options)
    {
        $query->when($options->application_uuid, function ($query, $uuid) {
            return $query->where('application_uuid', $uuid);
        });
