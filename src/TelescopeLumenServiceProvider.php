...
class TelescopeLumenServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // to make sure commands gets evented
        if ($this->app->runningInConsole()) {
            $kernel = $this->app['Illuminate\Contracts\Console\Kernel'];

            $symfonyDispatcher = new EventDispatcher;
            $symfonyDispatcher->addListener(ConsoleEvents::TERMINATE, function (ConsoleTerminateEvent $event) {
                $this->app[Dispatcher::class]->dispatch(
                    new CommandFinished($event->getCommand()->getName(), $event->getInput(), $event->getOutput(), $event->getExitCode())
                );
            });

            $kernel->setSymfonyDispatcher($symfonyDispatcher);
        }
    ...
    private function registerPublishing()
    {
        if ($this->app->runningInConsole()) {
            ...
            $this->publishes([
                __DIR__.'/../public' => rtrim(app()->basePath('public/vendor/telescope'), '/'),
